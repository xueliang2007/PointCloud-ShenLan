...
def octree_recursive_build(root, db, center, extent, point_indices, leaf_size, min_extent):
    ...
    if len(point_indices) <= leaf_size or extent <= min_extent:
        ...
        # 屏蔽开始
        root.is_leaf = False
        children_points_indices = [[] for i in range(8)]
        ...
        for point_idx in point_indices:
            ...
            children_points_indices[morton_code].append(point_idx)

        # determin child center and extent
        factor = [-0.5, 0.5]
        for i in range(8):
            ...
            root.children[i] = octree_recursive_build(root.children[i], db, child_center, child_extent,
                                                      children_points_indices[i], leaf_size, min_extent)
        # 屏蔽结束
    return root
...
def overlaps(query: np.ndarray, radius: float, octant: Octant):
    ...
    # if pass the above check, consider the case that the ball is contacting the face of the octant 判断球是否和面有接触
